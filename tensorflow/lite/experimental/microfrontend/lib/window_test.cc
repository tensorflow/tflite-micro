...
TEST(WindowTest, WindowState_CheckCoefficients) {
...
TEST(WindowTest, WindowState_CheckResidualInput) {
  WindowTestConfig config;
  struct WindowState state;
  EXPECT_TRUE(WindowPopulateState(&config.config_, &state, kSampleRate));
...
TEST(WindowTest, WindowState_CheckOutputValues) {
  WindowTestConfig config;
  struct WindowState state;
  EXPECT_TRUE(WindowPopulateState(&config.config_, &state, kSampleRate));
  size_t num_samples_read;
...
TEST(WindowTest, WindowState_CheckMaxAbsValue) {
  WindowTestConfig config;
  struct WindowState state;
  EXPECT_TRUE(WindowPopulateState(&config.config_, &state, kSampleRate));
  size_t num_samples_read;
...
TEST(WindowTest, WindowState_CheckConsecutiveWindow) {
  WindowTestConfig config;
  struct WindowState state;
  EXPECT_TRUE(WindowPopulateState(&config.config_, &state, kSampleRate));
  size_t num_samples_read;
...
TEST(WindowTest, WindowState_CheckNotEnoughSamples) {
  WindowTestConfig config;
  struct WindowState state;
  EXPECT_TRUE(WindowPopulateState(&config.config_, &state, kSampleRate));
  size_t num_samples_read;
  ...
  EXPECT_TRUE(WindowProcessSamples(
      ...
      sizeof(kFakeAudioData) / sizeof(kFakeAudioData[0]) - kWindowSamples,
      &num_samples_read));
  EXPECT_EQ(false, WindowProcessSamples(
