...
def build_test_model() -> bytes:
  ...
  act_in = sg.add_tensor(
    shape=(1, 4), dtype=tflite.TensorType.FLOAT32, name="act_in"
  )
  weights = sg.add_tensor(
    ...
    data=np.stack(
      [
        np.roll(np.array([-1.0, 0.0, 0.5, 1.0], dtype=np.float32), i)
        for i in range(8)
      ]
    ),
    name="weights",
  ...
  act_out = sg.add_tensor(
    shape=(1, 8), dtype=tflite.TensorType.FLOAT32, name="act_out"
  )
...
class TestConsumerGrouping(unittest.TestCase):
  def build(self, consumers: int) -> bytes:
    ...
    sg = model.add_subgraph()
    act = sg.add_tensor(
      shape=(64,), dtype=tflite.TensorType.FLOAT32, name="act"
    )
    const = sg.add_tensor(
      shape=(64,),
      dtype=tflite.TensorType.FLOAT32,
      data=np.tile(np.array([0.0, 1.0], dtype=np.float32), 32),
      name="const",
    )
  ...
  def build(self) -> bytes:
    ...
    sg.add_tensor(
      ...
      quantization=model_editor.Quantization(
        scales=[0.1, 0.2], zero_points=[0, 0], axis=0
      ),
      name="weights",
    )
    return bytes(model.build())
...
class TestSharedBuffer(unittest.TestCase):
  def build(self) -> bytes:
    ...
    sg.add_tensor(
      shape=(64,), dtype=tflite.TensorType.FLOAT32, buffer=shared, name="first"
    )
    sg.add_tensor(
      shape=(64,), dtype=tflite.TensorType.FLOAT32, buffer=shared, name="second"
    )
    return bytes(model.build())
...
class TestConstantInputs(unittest.TestCase):
  ...
  def setUpClass(cls):
    ...
    padded = sg.add_tensor(
      shape=(1, 8, 8, 4), dtype=tflite.TensorType.FLOAT32, name="padded"
    )
    sg.add_operator(
    ...
    out = sg.add_tensor(
      shape=(1, 6, 6, 4), dtype=tflite.TensorType.FLOAT32, name="out"
    )
    sg.add_operator(
    ...
    padded_values = sg.add_tensor(
      shape=(1, 8, 8, 4), dtype=tflite.TensorType.FLOAT32, name="padded_values"
    )
    sg.add_operator(
      opcode=tflite.BuiltinOperator.PAD,
