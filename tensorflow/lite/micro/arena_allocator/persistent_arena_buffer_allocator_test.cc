...
TEST(PersistentArenaBufferAllocatorTest, TestGetPersistentUsedBytes) {
  ...
  EXPECT_EQ(size1, allocator.GetPersistentUsedBytes());

  const size_t size2 = 15;
  allocator.AllocatePersistentBuffer(size2, 1);

  EXPECT_EQ(size1 + size2, allocator.GetPersistentUsedBytes());
}

// Test allocation shall fail if total memory exceeds the limit.
TEST(PersistentArenaBufferAllocatorTest,
     TestAllocatePersistBufferShallFailIfExceedLimit) {
  constexpr size_t arena_size = 1024;
  uint8_t arena[arena_size];
  tflite::PersistentArenaBufferAllocator allocator(arena, arena_size);
...
TEST(PersistentArenaBufferAllocatorTest,
  ...
  EXPECT_NE(persist2, nullptr);
  EXPECT_EQ(arena_size, allocator.GetPersistentUsedBytes());
}

// Test alignment works.
TEST(PersistentArenaBufferAllocatorTest, TestAllocatePersistBufferAligns) {
  constexpr size_t arena_size = 1024;
  uint8_t arena[arena_size];
  tflite::PersistentArenaBufferAllocator allocator(arena, arena_size);
  ...
  EXPECT_NE(persist1, nullptr);
  EXPECT_EQ((reinterpret_cast<std::uintptr_t>(persist1)) % alignment,
            static_cast<std::uintptr_t>(0));
  EXPECT_GE(allocator.GetPersistentUsedBytes(), size1);

  const size_t size2 = 16;
  uint8_t* persist2 = allocator.AllocatePersistentBuffer(size2, alignment);
  EXPECT_NE(persist2, nullptr);
  EXPECT_EQ((reinterpret_cast<std::uintptr_t>(persist2)) % alignment,
            static_cast<std::uintptr_t>(0));
  EXPECT_EQ(static_cast<size_t>(persist1 - persist2), size2);
  EXPECT_GE(allocator.GetPersistentUsedBytes(), size1);
}
