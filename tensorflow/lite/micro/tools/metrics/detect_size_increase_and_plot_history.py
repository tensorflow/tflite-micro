...
def _plot_and_detect_size_increase_for_binary(
  input_dir, output_dir, binary_name, threshold
):
  ...
  for index, name in enumerate(['text', 'data', 'total']):
    ...
    if err_msg_or_none is not None:
      threshold_messages.append(
        '%s failure: %s' % (binary_name, err_msg_or_none)
      )
...
def _subplot_and_detect_size_increase(
  subplot_axs, size_log, section_name, row, threshold
):
...
def _detect_size_increase_and_plot_history(
  input_dir, output_dir, binary_list, threshold_setting
):
  threshold_messages = []
...
if __name__ == '__main__':
  ...
  default_binary_list_string = (
    'keyword_benchmark,baseline_memory_footprint,interpreter_memory_footprint'
  )
  parser.add_argument(
    '--binary_list',
    nargs='?',
    ...
    help='binary list separated by comma (e.g. keyword_benchmark,baseline_memory_footprint)',
  )
  parser.add_argument(
    '--input_dir', help='Path to the size log file (e.g. ~/size_log'
  )
  ...
  _detect_size_increase_and_plot_history(
    args.input_dir, args.output_dir, binary_names, SIZE_THRESHOLD_SETTING
  )
