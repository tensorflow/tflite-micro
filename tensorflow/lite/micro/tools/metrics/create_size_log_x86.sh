...
function profile_a_binary() {
  ...
  raw_size=$(size ${GEN_FILES_DIR}/${binary})
  # Skip the title row
  ...
  echo "${binary}, ${text_size}, ${data_size}, ${bss_size}, ${total_size}" >> ${log}
}
...
function start_size_report() {
  ...
  echo "" >> ${log}
  echo "${START_HEADER}"  >> ${log}
  date >> ${log}
  echo "Profiled at ${HEAD_SHA}" >> ${log}
  echo "binary name, text, data, bss, total" >> ${log}
...
# Parameters:
# ${1} - (size) log file name
# ${2} - target binary name such as keyword_benchmark
function report_size() {
  ...
  local build_result=$?
  local binary_path="${target}_${target_arch}_${build_type}/bin/${binary}"
...
start_size_report ${SIZE_LOG}

report_size ${SIZE_LOG} keyword_benchmark
KEYWORD_BENCHMARK_STATUS=$?
report_size ${SIZE_LOG} baseline_memory_footprint
BASELINE_MEMORY_FOOTPRINT_STATUS=$?
report_size ${SIZE_LOG} interpreter_memory_footprint
INTERPRETER_MEMORY_FOOTPRINT_STATUS=$?

end_size_report ${SIZE_LOG}
