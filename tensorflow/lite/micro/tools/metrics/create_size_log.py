...
def _profile_a_binary(root_dir, binary_name, makefile_options, build_info):
  ...
  csv_path = os.path.join(
    root_dir,
    'data/continuous_builds/size_profiling',
    target_dir,
    "%s.csv" % binary_name,
  )
  ...
  process = subprocess.Popen(
    ["size", binary_path], stdout=subprocess.PIPE, stderr=subprocess.PIPE
  )
  stdout, stderr = process.communicate()
  if process.returncode != 0:
    raise RuntimeError(
      "size %s failed with \n\n %s" % (binary_name, stderr.decode())
    )
...
def _get_build_info(root_dir):
  ...
  git_process = subprocess.Popen(
    ["git", "rev-parse", "HEAD"], stdout=subprocess.PIPE, cwd=root_dir
  )
