...
_TENSOR_TYPE_NAMES = {
    ...
    for name in dir(schema_fb.TensorType) if not name.startswith('_')
}

_BUILTIN_OP_NAMES = {
    getattr(schema_fb.BuiltinOperator, name): name
    for name in dir(schema_fb.BuiltinOperator) if not name.startswith('_')
}
...
def analyze_model(model_path: str) -> None:
  ...
  for i in range(model.OperatorCodesLength()):
    ...
    if code == schema_fb.BuiltinOperator.CUSTOM:
      custom_name = (opcode.CustomCode().decode('utf-8')
                     if opcode.CustomCode() else 'CUSTOM')
  ...
  for sg_idx in range(num_subgraphs):
    ...
    outs_str = ', '.join(f'T#{sg.Outputs(i)}'
    ...
    for op_idx in range(sg.OperatorsLength()):
      ...
      op_ins = ', '.join(f'T#{op.Inputs(i)}' for i in range(op.InputsLength())
                         if op.Inputs(i) >= 0)
      op_outs = ', '.join(f'T#{op.Outputs(i)}'
    ...
    for t_idx in range(sg.TensorsLength()):
      ...
      buf_obj = (model.Buffers(buf_idx)
                 if buf_idx < model.BuffersLength() else None)
      buf_len = (buf_obj.DataLength()
                 if buf_obj and not buf_obj.DataIsNone() else 0)
      ro_str = f' RO {buf_len} bytes, buffer: {buf_idx}' if buf_len > 0 else ''
      print(f'  T#{t_idx}({name}) shape:{shape}, type:{ttype}{ro_str}')

  total_size = len(buf)
  data_size = sum(
      model.Buffers(i).DataLength() for i in range(model.BuffersLength())
  ...
  print(f'    Non-data buffer size: {non_data_size:10d} bytes'
        f' ({non_data_pct:05.2f} %)')
  print(
      f'  Total data buffer size: {data_size:10d} bytes ({data_pct:05.2f} %)')
