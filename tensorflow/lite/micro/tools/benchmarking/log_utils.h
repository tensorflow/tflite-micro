...
int GetMaxStringLength(const char strings[][kMaxStringLength], int count);
...
// Encodes a pointer to binary data as a base64 string. The output data should
// be (4/3 + 4) the size of the input. This will crash if the output buffer is
// too small for the required input.
// Args:
// - input: A pointer to input binary data.
// - output: A pointer to an output string buffer.
// - input_len: The size of the input data in bytes.
// - output_len: The size of the output buffer.
void Base64Encode(const uint8_t* input, char* output, size_t input_len,
                  size_t output_len);

// Checks to see if the encoded base64 buffer is long enough to support the
// input string.
bool CheckEncodedBase64Length(size_t input_len, size_t output_len);
...
void PrintTable(const char headers[kColumns][kMaxStringLength],
  ...
  for (int i = 0; i < kColumns; ++i) {
    max_column_width[i] = std::max(GetMaxStringLength(data[i], rows),
