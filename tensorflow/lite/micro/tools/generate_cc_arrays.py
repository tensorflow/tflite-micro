        ...
        return [
            ...
            (
                ...
                if "_int32.csv" in input_fname
                    ...
                    else "int8_t" if "_int8.csv" in input_fname else "float"
                )
            ),
        ]
    else:
...
def main():
    ...
    parser.add_argument(
        "output", help="base directory for all outputs or a cc or header to generate."
    )
    parser.add_argument(
        "inputs",
        nargs="+",
    ...
    if args.output.endswith(".cc") or args.output.endswith(".h"):
        ...
        for input_file in list(dict.fromkeys(args.inputs)):
            ...
            generate_file(
                output_cc_fname, generated_array_name, array_type, cc_array, size
            )
            generate_file(
                output_hdr_fname, generated_array_name, array_type, cc_array, size
            )
