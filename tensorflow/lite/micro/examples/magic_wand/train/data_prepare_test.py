...
class TestPrepare(unittest.TestCase):
    def setUp(self):  # pylint: disable=g-missing-super-call
        self.file = "./%s/output_%s_%s.txt" % (folders[0], folders[0], names[0]
    ...
    def test_prepare_data(self):
        ...
        self.assertEqual(list(self.data[-1]),
                         ["gesture", "accel_ms2_xyz", "name"])
    ...
    def test_generate_negative(self):
        ...
        for idx, data in enumerate(self.data):  # pylint: disable=undefined-variable, unused-variable
            if data["name"] == "negative6" or data[
                    "name"] == "negative7" or data["name"] == "negative8":
                generated_num += 1
