...
def convert_8to16_converter(keras_model, representative_dataset_gen):
    ...
    int8_model = convert_tfl_converter(keras_model, representative_dataset_gen, int16 = False)
...
class SimpleFCModelTest(test_util.TensorFlowTestCase):
    def testCompareWithStandardConversion(self):
        ...
        keras_model = create_simple_fc_model() # int16 fc is supported in tflite converter 
        tfl_converted_int16_model = convert_tfl_converter(keras_model,representative_dataset_gen, int16 = True)
        int8_converted_int16_model = convert_8to16_converter(keras_model,representative_dataset_gen)
        
        interpreter_tfl_converted = tflm_runtime.Interpreter.from_bytes(tfl_converted_int16_model)
        interpreter_tool_converted = tflm_runtime.Interpreter.from_bytes(int8_converted_int16_model)
        ...
        for _ in range(0, num_steps):
            ...
            self.assertLess(max_diff, 1e-4) # can not be the same since int8 model loses some range information
