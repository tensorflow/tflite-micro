...
void RunModel(const uint8_t* model, const int32_t* input0,
  ...
  EXPECT_EQ(input_tensor0->bytes, input0_size * sizeof(int32_t));
...
TEST(IntegrationTests, transpose_conv0_test) {
...
TEST(IntegrationTests, transpose_conv1_test) {
  tflite::micro::RunModel(
...
TEST(IntegrationTests, transpose_conv2_test) {
  tflite::micro::RunModel(
...
TEST(IntegrationTests, transpose_conv3_test) {
  tflite::micro::RunModel(
...
TEST(IntegrationTests, transpose_conv4_test) {
  tflite::micro::RunModel(
