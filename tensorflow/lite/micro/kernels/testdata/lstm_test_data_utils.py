...
class QuantizedTensor:
  ...
  def __init__(self, float_data, scale, zero_point, symmetry, num_bits=8):
    ...
    self.quantized_data = quantize_data(float_data, scale, zero_point, num_bits)

  @property
  def dequantized_data(self):
    """Dequantize the quantized tensor data back to floating point"""
    return dequantize_data(
      self.quantized_data, self.scale, self.zero_point
    ).flatten()
...
class QuantizedGateParams:
  ...
  def quantize_bias_data(self, input_scale):
    ...
    return quantize_data(
      self.bias_data_float, bias_scale, 0, self.bias_num_bits
    )
  ...
  def compute_recurrent_bias(self, recurrent_zp):
    ...
    return self.fold_zeropoint(
      self.recurrent_weight.quantized_data, recurrent_zp
    )

  def effective_activation_scale(self, input_scale):
    # Combine input scale with output scale. Used for fc calculation
    return (
      self.activation_weight.scale * input_scale / self.nonlinear_input_scale
    )

  def effective_recurrence_scale(self, recurrent_scale):
    # Combine input scale with output scale. Used for fc calculation
    return (
      self.recurrent_weight.scale * recurrent_scale / self.nonlinear_input_scale
    )


def assemble_quantized_tensor(
  float_data, min_val, max_val, symmetry, num_bits=8
):
  ...
  return QuantizedTensor(
    float_data, scale, zp, symmetry=symmetry, num_bits=num_bits
  )
...
def create_gate_params(gate_parameters, model_config, modulation=False):
  ...
  activation_weight_data = np.array(
    gate_parameters['activation_weight_data']
  ).reshape((shape_info['input_dim'], shape_info['state_dim']))
  ...
  recurrent_weight_data = np.array(
    gate_parameters['recurrent_weight_data']
  ).reshape((shape_info['input_dim'], shape_info['state_dim']))
...
def gate_calculation(input, hidden_state, gate_params, debug=False):
  ...
  # Quantized Version
  input_fc = np.dot(
    gate_params.activation_weight.quantized_data, input.quantized_data
  )
  input_fc += gate_params.compute_activation_bias(input.scale, input.zero_point)
  input_fc = rescale(
    input_fc,
    gate_params.effective_activation_scale(input.scale),
    0,
    gate_params.cell_num_bits,
  )
  recurrent_fc = np.dot(
    gate_params.recurrent_weight.quantized_data, hidden_state.quantized_data
  )
  recurrent_fc += gate_params.compute_recurrent_bias(hidden_state.zero_point)
  recurrent_fc = rescale(
    recurrent_fc,
    gate_params.effective_recurrence_scale(hidden_state.scale),
    0,
    gate_params.cell_num_bits,
  )

  before_activation = clip_range(
    input_fc + recurrent_fc, gate_params.cell_num_bits
  )

  # Float Version
  float_result = np.dot(
    gate_params.activation_weight.float_data, input.float_data
  )
  float_result += np.dot(
    gate_params.recurrent_weight.float_data, hidden_state.float_data
  )
  ...
  if debug:
    ...
    dequantized_res = dequantize_data(
      before_activation, gate_params.nonlinear_input_scale
    )
  ...
  if debug:
    dequantized_res = dequantize_data(
      activated, gate_params.nonlinear_output_scale
    )
...
class QuantizedLSTMDebugger(object):
  ...
  def invoke(self, input_tensor, debug=False):
    ...
    self.cell_state_tensor.float_data = (
      prev_cell_state_float * forget_gate_float
    )
    ...
    if debug:  # Hidden/cell state will be updated, break up the debug to record the intermediate state
      print('======================One Step LSTM======================')
      ...
      print(f'Dequantized: {self.cell_state_tensor.dequantized_data.flatten()}')
      ...
      dequantized_val = dequantize_data(
        gated_input_quant, self.cell_state_tensor.scale, 0
      )
      print(f'Dequantized: {dequantized_val.flatten()}')
    ...
    if debug:
      ...
      diff = abs(
        self.hidden_state_tensor.dequantized_data - gated_output_float.flatten()
      )
  ...
  def _calculate_effective_cell_scale(self):
    return (
      self.input_gate_params.nonlinear_output_scale
      * self.modulation_gate_params.nonlinear_output_scale
      / self.cell_state_tensor.scale
    )
  ...
  def _apply_cell_clip(self):
    ...
    if (
      cell_vals.max() > self.quantized_cell_clip
    ...
    self.cell_state_tensor.quantized_data = np.round(
      np.clip(cell_vals, -self.quantized_cell_clip, self.quantized_cell_clip)
    )
