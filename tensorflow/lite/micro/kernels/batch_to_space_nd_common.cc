...
const int kBatchToSpaceNDInputTensor = 0;
const int kBatchToSpaceNDBlockShapeTensor = 1;
const int kBatchToSpaceNDCropsTensor = 2;
const int kBatchToSpaceNDOutputTensor = 0;
...
TfLiteStatus BatchToSpaceNDPrepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  const TfLiteTensor* input = GetInput(context, node, kBatchToSpaceNDInputTensor);
  TfLiteTensor* output = GetOutput(context, node, kBatchToSpaceNDOutputTensor);
