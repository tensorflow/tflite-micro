...
TfLiteRegistration_V1* Register_ONE_HOT() {
  static TfLiteRegistration_V1 r = {/*init, free, prepare, eval*/};
  return &r;
}
...
// // Convenience utility for destructuring a node into the appropriate tensors
// and
// // data for the op. Note that this destructuring is quite cheap, so we can
// avoid
...
//     return;
//   }
//   const int suffix_dim_size = NumElements(op_context.indices) /
...
//   TF_LITE_ENSURE_TYPES_EQ(context, op_context.on_value->type,
//   op_context.dtype); TF_LITE_ENSURE_TYPES_EQ(context,
