  ...
  if (output.quantization.type == kTfLiteAffineQuantization &&
    ...
    if (num_channels_ > 1) {
      ...
      const size_t bufsize =
          quantization->zero_point->size * sizeof(*zero_points_);
      zero_points_ = static_cast<int32_t*>(
...
void DecodeStatePrune::DecompressToBuffer(void* vp) {
  if (num_channels_ > 1) {
    DecompressToBufferPerChannel<T>(vp);
    return;
  }
...
template <typename T>
void DecodeStatePrune::DecompressToBufferPerChannel(void* vp) {
  ScopedMicroProfiler scoped_profiler(__func__, micro_profiler_);
  ...
  if (use_alternate_axis_) {
    ...
    while (count > 0) {
      for (size_t channel = 0; channel < max_channels; channel++) {
        const int32_t zp = zero_points_[channel];
...
template void DecodeStatePrune::DecompressToBufferPerChannel<int8_t>(void*);
template void DecodeStatePrune::DecompressToBufferPerChannel<int16_t>(void*);
template void DecodeStatePrune::DecompressToBufferPerChannel<int32_t>(void*);
template void DecodeStatePrune::DecompressToBufferPerChannel<int64_t>(void*);
