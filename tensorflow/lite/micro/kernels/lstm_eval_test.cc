...
void TestGateOutputFloat(const int batch_size,
  ...
  const int output_size = batch_size * kStateDimension;
...
TF_LITE_MICRO_TEST(CheckGateOutputFloat) {
  ...
  tflite::testing::TestGateOutputFloat(
      batch_size, tflite::testing::kForgetGateParameters, kTfLiteActSigmoid,
      input_data, recurrent_data, expected_forget_gate_output);
  ...
  tflite::testing::TestGateOutputFloat(
      batch_size, tflite::testing::kInputGateParameters, kTfLiteActSigmoid,
      input_data, recurrent_data, expected_input_gate_output);
  ...
  const float expected_output_gate_output[] = {0.6456563062257954,
                                               0.6456563062257954};
  tflite::testing::TestGateOutputFloat(
      batch_size, tflite::testing::kOutputGateParameters, kTfLiteActSigmoid,
      input_data, recurrent_data, expected_output_gate_output);
  ...
  const float expected_modulation_gate_output[] = {0.5370495669980353,
                                                   0.5370495669980353};
  tflite::testing::TestGateOutputFloat(
      batch_size, tflite::testing::kModulationGateParameters, kTfLiteActTanh,
      input_data, recurrent_data, expected_modulation_gate_output);
}
...
TF_LITE_MICRO_TEST(CheckCellUpdateFloat) {
  ...
  const int cell_size = batch_size * tflite::testing::kStateDimension;
  ...
  tflite::lstm_internal::UpdateLstmCellFloat(
      batch_size, tflite::testing::kStateDimension, cell_data, input_gate,
      forget_gate, cell_gate, /*use_cifg=*/false, /*clip=*/10);
...
TF_LITE_MICRO_TEST(CheckOutputFloat) {
  ...
  const int output_size = tflite::testing::kStateDimension * batch_size;
  float output[output_size];
  float scratch[output_size];

  tflite::lstm_internal::CalculateLstmOutputFloat(
      batch_size, tflite::testing::kStateDimension,
