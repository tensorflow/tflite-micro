...
struct TestingInfo {
  ...
  uint8_t* compressed;
  T* value_table;
  std::array<uint8_t, tflite::DecodeState::kDcmSizeInBytes>*
      decode_common_metadata;
  size_t bit_width;
...
struct TestingData {
  ...
  // ancillary tensor data storage
  alignas(MicroArenaBufferAlignment()) std::array<
      uint8_t, tflite::DecodeState::kDcmSizeInBytes> decode_common_metadata;
  T value_table[kValueTableSize];
...
TfLiteStatus TestDecompression(TestingInfo<T>* info) {
  ...
  *info->decode_common_metadata = {
  ...
  const int encoded_dims_array[] = {
      1, static_cast<int>(((info->total_elements * info->bit_width) + 7) / 8)};
  const TfLiteIntArray* const encoded_dims =
      tflite::testing::IntArrayFromInts(encoded_dims_array);
  const TensorInDatum tid_encode = {
      info->compressed,
      *encoded_dims,
  };
  ...
  const int ancillary_dims_array[] = {
      1, static_cast<int>((info->total_value_table_elements * sizeof(T)) +
                          tflite::DecodeState::kDcmSizeInBytes)};
  const TfLiteIntArray* const ancillary_dims =
      tflite::testing::IntArrayFromInts(ancillary_dims_array);
  const TensorInDatum tid_ancillary = {
      info->decode_common_metadata,
      *ancillary_dims,
  };
  ...
  tflite::testing::TestDecode<kNumInputs, kNumOutputs>(
