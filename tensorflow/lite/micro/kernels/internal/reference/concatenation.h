...
#ifndef TENSORFLOW_LITE_MICRO_KERNELS_INTERNAL_REFERENCE_CONCATENATION_H_
#define TENSORFLOW_LITE_MICRO_KERNELS_INTERNAL_REFERENCE_CONCATENATION_H_
  ...
  for (int64_t k = 0; k < outer_size; k++) {
    for (int i = 0; i < inputs_count; ++i) {
      ...
      if (input_zeropoint[i] == output_zeropoint &&
        ...
        for (int64_t j = 0; j < copy_size; ++j) {
          const int32_t value = static_cast<int32_t>(tflite::micro::TfLiteRound(
...
#endif  // TENSORFLOW_LITE_MICRO_KERNELS_INTERNAL_REFERENCE_CONCATENATION_H_
