...
==============================================================================*/
#ifndef TENSORFLOW_LITE_MICRO_KERNELS_INTERNAL_REFERENCE_REDUCE_H_
#define TENSORFLOW_LITE_MICRO_KERNELS_INTERNAL_REFERENCE_REDUCE_H_
...
inline bool CheckedElementCount(const int* dims, const int num_dims,
  ...
  for (int idx = 0; idx < num_dims; ++idx) {
    ...
    if (current > 0 && count > std::numeric_limits<size_t>::max() / current) {
      return false;
    }
    count *= current;
  }
...
inline bool CheckedReducedElementCount(const int* dims, const int num_dims,
  ...
  for (int idx = 0; idx < num_axes; ++idx) {
    ...
    if (count > 0 && current > std::numeric_limits<T>::max() / count) {
      return false;
    }
    count *= current;
  }
...
inline void Mean(const tflite::micro::MeanParams& op_params,
...
#endif  // TENSORFLOW_LITE_MICRO_KERNELS_INTERNAL_REFERENCE_REDUCE_H_
