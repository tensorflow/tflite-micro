...
TfLiteStatus ConvPrepareVision(TfLiteContext* context, TfLiteNode* node) {
  ...
  if ((input->type == kTfLiteInt8) && (params.stride_width == 1) &&
  ...
  if (data->enableXtensaKernel) {
    ...
    status = xiConvSetContext(
        data->pContext, data->contextSize, input_depth, input_width,
    ...
    status =
        xiConvGetMemReqd_Coeff(data->pContext, data->contextSize, &coeffSize);
    if (!status && coeffSize) {
