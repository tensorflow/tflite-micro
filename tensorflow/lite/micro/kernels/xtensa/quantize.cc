...
TfLiteStatus EvalXtensa(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (input->type == kTfLiteFloat32) {
    ...
    switch (output->type) {
      ...
      {
#if defined(HIFIMINI)
        AffineQuantize(data->scale_multiplier, data->zero_point,
        ...
        TF_LITE_ENSURE_EQ(
            ...
            data->requantize_output_multiplier, size),
        0);
#elif defined(HIFI5)
        int size = ElementCount(*input->dims);
        TF_LITE_ENSURE_EQ(
      ...
      case kTfLiteInt32:
      {
        int size = ElementCount(*input->dims);
#if defined(HIFIMINI)
        int32_t zero_point = data->zero_point;
#else
        int32_t zero_point = data->quantization_params.zero_point;
#endif // defined(HIFIMINI)
#if defined(HIFI5)
      TF_LITE_ENSURE_EQ(context,
        ...
        reference_ops::Requantize(
            tflite::micro::GetTensorData<int16_t>(input), size,
            data->requantize_output_multiplier, data->requantize_output_shift,
            data->input_zero_point, zero_point,
            tflite::micro::GetTensorData<int32_t>(output));
#endif // defined(HIFI5)
        return kTfLiteOk;
      }
    ...
    switch (output->type) {
      ...
      TF_LITE_ENSURE_EQ(
          ...
          xa_nn_elm_requantize_asym8s_asym32s(
              ...
              zero_point, data->requantize_output_shift,
              data->requantize_output_multiplier, size),
          0);
#else
      reference_ops::Requantize(tflite::micro::GetTensorData<int8_t>(input),
                                size, data->requantize_output_multiplier,
                                data->requantize_output_shift,
                                data->input_zero_point, zero_point,
                                tflite::micro::GetTensorData<int32_t>(output));
#endif  // defined(HIFI5)
