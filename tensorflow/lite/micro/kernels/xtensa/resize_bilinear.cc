...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (output->type == kTfLiteFloat32) {
    ...
    int32_t output_height = tflite::micro::GetTensorData<int32_t>(size)[Offset(output_size_shape, 0, 0, 0, 0)];
    int32_t output_width = tflite::micro::GetTensorData<int32_t>(size)[Offset(output_size_shape, 0, 0, 0, 1)];
    ...
    int32_t height_shift = 0;
    int32_t width_shift = 0;
    if (op_params.half_pixel_centers) {
      height_shift = (int32_t)(height_scale_10 / 2 - (1 << 9));
      width_shift = (int32_t)(width_scale_10 / 2 - (1 << 9));
    }
    
    xa_nn_resize_bilinear_8_8(output_ptr, input_ptr, batches, input_height, input_width, depth, 
                      batches, output_height, output_width, depth, height_scale_10, width_scale_10,
