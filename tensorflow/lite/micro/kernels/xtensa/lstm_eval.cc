...
#define LUT_BASED_ACTVN

namespace tflite {

#if defined(HIFI5)
extern void calc_cell_state_without_cifg(int16_t* cell_state,
                                         const int16_t* forget_gate,
                                         const int16_t* cell_gate,
                                         const int16_t* input_gate, int shift1,
                                         int shift2, int clip, int num_elms);

extern void calc_cell_state_with_cifg(int16_t* cell_state, const int16_t* forget_gate,
                                      const int16_t* cell_gate, int shift1, int shift2,
                                      int clip, int num_elms);

extern void xa_nn_elm_mul_16x16_asym8s(int8_t* output, const int16_t* input_1,
...
#if !defined(HIFI5) && !defined(HIFI4)
  tensor_utils::ApplyTanh(15 + cell_state_scale, cell_state, n_batch,
                                n_cell, scratch0);
#else
#if defined(LUT_BASED_ACTVN)
      ...
      RuntimeShape tanh_inp_shape = RuntimeShape(1, &dims_data);
      reference_integer_ops::Tanh(0, tanh_input_left_shift, tanh_inp_shape, cell_state, tanh_inp_shape,
                                  scratch0);
#else
