...
TfLiteStatus DepthwiseConvPrepareVision(TfLiteContext* context,
  ...
  if (data->enableXtensaKernel) {
    ...
    status = xiDepthwiseConvSetContext(
        data->pContext, data->contextSize, inputD, inputW, inputH, outputD,
    ...
    status = xiDepthwiseConvGetMemReqd_Coeff(data->pContext, data->contextSize,
                                             &coeffSize);
    if (!status && coeffSize) {
    ...
    status = xiDepthwiseConvDoCoeffReorder(
        data->pContext, data->contextSize, (uint8_t*)data->reordCoeffnBias,
        data->reordCoeffnBiasSize, (uint8_t*)GetTensorData<uint8_t>(filter),
...
TfLiteStatus DepthwiseConvEvalVision(TfLiteContext* context, TfLiteNode* node) {
  ...
  const XtensaDepthwiseConvOpData& data =
      *(static_cast<const XtensaDepthwiseConvOpData*>(node->user_data));
  ...
  if (data.enableXtensaKernel) {
    ...
    xiDepthwiseConv(data.pContext, data.contextSize,
                    ...
                    output_size, data.reordCoeffnBias, data.reordCoeffnBiasSize,
