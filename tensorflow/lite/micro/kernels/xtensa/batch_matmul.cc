...
struct OpData {
  ...
  // Scratch buffer is required for batch_matmul HiFi implementation
...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  int orig_dims_count = extended_lhs_shape.DimensionsCount();
  for(int i = 0; i < orig_dims_count; i++){
    mat_inp1_shape[4-i] = extended_lhs_shape.Dims(orig_dims_count-1-i);
  }
  orig_dims_count = extended_rhs_shape.DimensionsCount();
  for(int i = 0; i < orig_dims_count; i++){
    mat_inp2_shape[4-i] = extended_rhs_shape.Dims(orig_dims_count-1-i);
  }
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  if( (lhs->type == kTfLiteInt8) || (lhs->type == kTfLiteInt16)){
    ...
    int mat_inp1_shape[] = {1, 1, 1, 1, 1};
    int mat_inp2_shape[] = {1, 1, 1, 1, 1};
    int mat_out_shape[] = {1, 1, 1, 1, 1};

    int orig_dims_count = orig_lhs_shape.DimensionsCount();
    for(int i = 0; i < orig_dims_count; i++){
      mat_inp1_shape[4-i] = orig_lhs_shape.Dims(orig_dims_count-1-i);
    }
    orig_dims_count = orig_rhs_shape.DimensionsCount();
    for(int i = 0; i < orig_dims_count; i++){
      mat_inp2_shape[4-i] = orig_rhs_shape.Dims(orig_dims_count-1-i);
    }
    orig_dims_count = output_shape.DimensionsCount();
    for(int i = 0; i < orig_dims_count; i++){
      mat_out_shape[4-i] = output_shape.Dims(orig_dims_count-1-i);
    }
    ...
    if(lhs->type == kTfLiteInt8){
      TF_LITE_ENSURE_EQ(
            ...
            xa_nn_batch_matmul_asym8sxasym8s_asym8s(
                tflite::micro::GetTensorData<int8_t>(output), mat_out_shape,
                tflite::micro::GetTensorData<int8_t>(rhs), mat_inp2_shape,
                tflite::micro::GetTensorData<int8_t>(lhs), mat_inp1_shape,
                !adj_y, adj_x, 
                -op_data->quantization->rhs_zero_point, -op_data->quantization->lhs_zero_point,
                op_data->quantization->output_multiplier, op_data->quantization->output_shift,
                op_data->quantization->output_zero_point, p_scratch),
          0);
        return kTfLiteOk;
    } else if (lhs->type == kTfLiteInt16) {
      TF_LITE_ENSURE_EQ(
            context,
            xa_nn_batch_matmul_sym16sxsym16s_sym16s(
                tflite::micro::GetTensorData<int16_t>(output), mat_out_shape,
                tflite::micro::GetTensorData<int16_t>(rhs), mat_inp2_shape,
                tflite::micro::GetTensorData<int16_t>(lhs), mat_inp1_shape,
                !adj_y, adj_x, 
                0, 0,
                op_data->quantization->output_multiplier, op_data->quantization->output_shift,
                0, p_scratch),
