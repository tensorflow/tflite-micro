...
TfLiteStatus Eval_8(TfLiteContext* context, TfLiteNode* node) {
  ...
  dispatcher->Initialize(fully_connected_thread_worker, n_th, op->stack_size,
                         stack);
  ...
  for (int i_cg = 0; i_cg < op->execution_plan.changrps.size(); i_cg++) {
    ...
    dispatcher_args[i_th] = &thread_data[i_th];

    i_th++;

    if (i_th == n_th) {
      dispatcher->Invoke(dispatcher_args, i_th);
  ...
  dispatcher->Invoke(dispatcher_args,
                     i_th);  // finish up any added tasks
