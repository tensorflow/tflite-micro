...
void ValidateSoftmaxGoldens(TfLiteTensor* tensors, const int tensor_count,
  ...
  for (int i = 0; i < output_dims_count; ++i) {
    EXPECT_NEAR(expected_output[i], output_data[i], tolerance);
  }
}
...
TEST(SoftmaxTest, Softmax1DQuantizedInt8ShouldMatchGolden) {
...
TEST(SoftmaxTest, Softmax1DQuantizedInt16ShouldMatchGolden) {
  const float input_scale = 0.1f;
  const int input_zero_point = 0;
...
TEST(SoftmaxTest, Softmax2DQuantizedInt16ShouldMatchGolden) {
  const float input_scale = 0.1f;
  const int input_zero_point = 0;
...
TEST(SoftmaxTest, Softmax3DQuantizedInt8ShouldMatchGolden) {
  const float input_scale = 0.1f;
  const int input_zero_point = 0;
...
TEST(SoftmaxTest, Softmax3DQuantizedInt16ShouldMatchGolden) {
  const float input_scale = 0.1f;
  const int input_zero_point = 0;
...
TEST(SoftmaxTest, Softmax4DQuantizedInt8ShouldMatchGolden) {
  const float input_scale = 0.1f;
  const int input_zero_point = 0;
...
TEST(SoftmaxTest, Softmax4DQuantizedInt16ShouldMatchGolden) {
  const float input_scale = 0.1f;
  const int input_zero_point = 0;
...
TEST(SoftmaxTest, Softmax2DQuantizedInt8InputInt16OutputShouldMatchGolden) {
  const float input_scale = 0.1f;
  const int input_zero_point = 0;
