...
inline const Character (&EnumValuesCharacter())[7] {
  static const Character values[] = { Character_NONE,     Character_MuLan,
                                      Character_Rapunzel, Character_Belle,
                                      Character_BookFan,  Character_Other,
                                      Character_Unused };
  return values;
}
...
struct CharacterUnion {
  ...
  AttackerT *AsMuLan() {
    return type == Character_MuLan ? reinterpret_cast<AttackerT *>(value)
                                   : nullptr;
  }
  const AttackerT *AsMuLan() const {
    return type == Character_MuLan ? reinterpret_cast<const AttackerT *>(value)
                                   : nullptr;
  }
  Rapunzel *AsRapunzel() {
    return type == Character_Rapunzel ? reinterpret_cast<Rapunzel *>(value)
                                      : nullptr;
  }
  ...
  BookReader *AsBelle() {
    return type == Character_Belle ? reinterpret_cast<BookReader *>(value)
                                   : nullptr;
  }
  const BookReader *AsBelle() const {
    return type == Character_Belle ? reinterpret_cast<const BookReader *>(value)
                                   : nullptr;
  }
  BookReader *AsBookFan() {
    return type == Character_BookFan ? reinterpret_cast<BookReader *>(value)
                                     : nullptr;
  }
  ...
  std::string *AsOther() {
    return type == Character_Other ? reinterpret_cast<std::string *>(value)
                                   : nullptr;
  }
  ...
  std::string *AsUnused() {
    return type == Character_Unused ? reinterpret_cast<std::string *>(value)
                                    : nullptr;
  }
...
inline const Gadget (&EnumValuesGadget())[3] {
  static const Gadget values[] = { Gadget_NONE, Gadget_FallingTub,
                                   Gadget_HandFan };
  return values;
}
...
struct GadgetUnion {
  ...
  GadgetUnion(GadgetUnion &&u) FLATBUFFERS_NOEXCEPT : type(Gadget_NONE),
                                                      value(nullptr) {
    std::swap(type, u.type);
    std::swap(value, u.value);
  }
  ...
  template<typename T> void Set(T &&val) {
    ...
    if (type != Gadget_NONE) { value = new RT(std::forward<T>(val)); }
  }
  ...
  FallingTub *AsFallingTub() {
    return type == Gadget_FallingTub ? reinterpret_cast<FallingTub *>(value)
                                     : nullptr;
  }
  ...
  HandFanT *AsHandFan() {
    return type == Gadget_HandFan ? reinterpret_cast<HandFanT *>(value)
                                  : nullptr;
  }
  const HandFanT *AsHandFan() const {
    return type == Gadget_HandFan ? reinterpret_cast<const HandFanT *>(value)
                                  : nullptr;
  }
};
...
inline bool operator==(const Rapunzel &lhs, const Rapunzel &rhs) {
  return (lhs.hair_length() == rhs.hair_length());
}
...
inline bool operator==(const BookReader &lhs, const BookReader &rhs) {
  return (lhs.books_read() == rhs.books_read());
}
...
inline bool operator==(const AttackerT &lhs, const AttackerT &rhs) {
  return (lhs.sword_attack_damage == rhs.sword_attack_damage);
}
...
inline bool operator==(const MovieT &lhs, const MovieT &rhs) {
  return (lhs.main_character == rhs.main_character) &&
...
inline flatbuffers::Offset<Movie> CreateMovie(
  ...
  auto _characters_type =
      _o->characters.size()
          ? _fbb.CreateVector<uint8_t>(
                _o->characters.size(),
                [](size_t i, _VectorArgs *__va) {
                  return static_cast<uint8_t>(__va->__o->characters[i].type);
                },
                &_va)
          : 0;
...
inline flatbuffers::unique_ptr<MovieT> UnPackSizePrefixedMovie(
  ...
  return flatbuffers::unique_ptr<MovieT>(
      GetSizePrefixedMovie(buf)->UnPack(res));
}
