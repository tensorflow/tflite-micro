...
class JavaTest {
  ...
  static void TestBuilderBasics(FlatBufferBuilder fbb, boolean sizePrefix) {
    ...
    int testArrayOfString = Monster.createTestarrayofstringVector(
    ...
    if (sizePrefix) {
      TestEq(ByteBufferUtil.getSizePrefix(dataBuffer) + SIZE_PREFIX_LENGTH, dataBuffer.remaining());
  ...
  static void TestVectorOfUnions() {
    ...
    Movie.finishMovieBuffer(fbb,
        Movie.createMovie(fbb, (byte) 0, (byte) 0,
  ...
  public static void testFlexBuffersTest(FlexBuffersBuilder builder) {
    ...
    TestEq(mymap.keys().get(0),
        m.keys().get(
            0)); // These should be equal by pointer equality, since key and value are shared.
  ...
  public static void testDeprecatedTypedVectorString() {
    ...
    byte[] data = new byte[] {0x03, 0x61, 0x62, 0x63, 0x00, 0x03, 0x61, 0x62, 0x63, 0x00, 0x03,
        0x61, 0x62, 0x63, 0x00, 0x03, 0x61, 0x62, 0x63, 0x00, 0x04, 0x14, 0x10, 0x0c, 0x08, 0x04,
        0x3c, 0x01};
  ...
  static void TestObject(MonsterT monster) {
    ...
    for (int i = 0; i < inv.length; i++) TestEq(expInv[i], inv[i]);
