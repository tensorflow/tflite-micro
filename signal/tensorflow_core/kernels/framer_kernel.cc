...
class FramerOp : public tensorflow::OpKernel {
  ...
  void Compute(tensorflow::OpKernelContext* context) override {
    ...
    if (!initialized_) {
      ...
      for (int i = 0; i < outer_dims_; i++) {
        ...
        circular_buffers_[i] = tflm_signal::CircularBufferInit(capacity, state_, state_size);
        if (prefill_) {
          tflm_signal::CircularBufferWriteZeros(circular_buffers_[i],
    ...
    for (int i = 0; i < outer_dims_; i++) {
      for (int frame = 0; frame < n_frames_; frame++) {
        ...
        tflm_signal::CircularBufferWrite(circular_buffers_[i],
                            &(input_tensor.flat<int16_t>().data())[input_index],
                            frame_step_);
        if (tflm_signal::CircularBufferAvailable(circular_buffers_[i]) >= frame_size_) {
          tflm_signal::CircularBufferGet(circular_buffers_[i], frame_size_,
                            &(reinterpret_cast<int16_t*>(
                                output_tensor->data()))[output_index]);
          tflm_signal::CircularBufferDiscard(circular_buffers_[i], frame_step_);
        } else {
          output_valid = false;
        }
      }
 ...
 private:
  ...
  std::vector<tflm_signal::CircularBuffer*> circular_buffers_;
};
