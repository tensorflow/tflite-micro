...
struct TfLiteSignalFramerParams {
  int32_t frame_size;
  int32_t frame_step;
  int32_t outer_dims;
  int32_t n_frames;
...
void ResetState(TfLiteSignalFramerParams* params) {
...
void* Init(TfLiteContext* context, const char* buffer, size_t length) {
  ...
  auto* params =
      static_cast<TfLiteSignalFramerParams*>(context->AllocatePersistentBuffer(
          context, sizeof(TfLiteSignalFramerParams)));
...
TfLiteStatus Prepare(TfLiteContext* context, TfLiteNode* node) {
  ...
  auto* params = reinterpret_cast<TfLiteSignalFramerParams*>(node->user_data);
...
void Reset(TfLiteContext* context, void* buffer) {
  ResetState(static_cast<TfLiteSignalFramerParams*>(buffer));
}
