...
class FramerKernelRunner {
 public:
  FramerKernelRunner(int* input_dims_data, int16_t* input_data,
    ...
    tensors_[2] = tflite::testing::CreateTensor(
        output_ready,
        tflite::testing::IntArrayFromInts(output_ready_dims_data));
 ...
 private:
  ...
  TFLMRegistration* registration_ = tflite::tflm_signal::Register_FRAMER();
...
TF_LITE_MICRO_TEST(FramerTest_3_1_0) {
  ...
  TestFramer(input_dims_data, &input_data, output_dims_data, golden,
...
TF_LITE_MICRO_TEST(TestReset) {
  ...
  int16_t input_data;
  int16_t output_data[kOutputSize];
  TestFramerReset(input_dims_data, &input_data, output_dims_data, golden,
                  sizeof(golden) / sizeof(int16_t), output_ready_dims_data,
                  g_gen_data_3_1_0_framer, g_gen_data_size_3_1_0_framer,
                  output_data);
}
