...
class OverlapAddOpTest(parameterized.TestCase, tf.test.TestCase):
  ...
  def testOverlapAddValidInput(self, dtype):
    ...
    while frame_index < frame_num:
      self.RunOverlapAdd(
          interpreters[0], [input_frames[frame_index]],
          1,
          expected_output_frames_step_1[frame_index],
          dtype=dtype)
      self.RunOverlapAdd(
          interpreters[1], [input_frames[frame_index]],
          2,
          expected_output_frames_step_2[frame_index],
          dtype=dtype)
      self.RunOverlapAdd(
          interpreters[2], [input_frames[frame_index]],
          3,
          expected_output_frames_step_3[frame_index],
          dtype=dtype)
      self.RunOverlapAdd(
          interpreters[3], [input_frames[frame_index]],
          4,
          expected_output_frames_step_4[frame_index],
          dtype=dtype)
      self.RunOverlapAdd(
          interpreters[4], [input_frames[frame_index]],
          5,
  ...
  def testOverlapAddNframes5(self, dtype):
    ...
    expected_output_frames_step_4 = np.array([
        1, -5, 4, 2, 11, 15, -44, 27, 50, -19, 79, 8, -134, 17, 65, 18, -98, 33,
        -66, -19
    ])
